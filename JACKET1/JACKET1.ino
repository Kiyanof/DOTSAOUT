...
byte UIDS[4][4] = { {0x1C, 0x3E, 0x3D, 0x4A},
                    {0x93, 0x99, 0xCC, 0x18},
                    {0x4A, 0xD0, 0x72, 0xBE},
                    {0x9C, 0xF8, 0x17, 0x4A}
};
...
unsigned long timerDelay3 = 3000;
...
void setup() {
  ...
  initWiFi();
    ...
    mfrc522.PCD_Init(SELECT, RST_PIN);
...
void loop() {
    ...
    if(corome != prevCorom && ((millis() - lastTime3) > timerDelay3) && !activated){
      events.send(String(corome).c_str(),"corom",millis()); 
      Serial.println("corom");  
      lastTime3 = millis();
      prevCorom = corome;  
        digitalWrite(SKILL_RELAY, HIGH);
        delay(100);
